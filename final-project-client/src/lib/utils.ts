...
export const ALGERIAN_PHONE_REGEX =
...
export const STATUS_CONFIG: Record<
  string,
  { label: string; labelAr: string; className: string; dot: string }
> = {
  "In Transit": {
    label: "En Transit",
    labelAr: "في العبور",
...
export function buildImageUrl(path: string | undefined): string {
  ...
  return `${process.env.NEXT_PUBLIC_UPLOADS_URL}${path.startsWith("/") ? "" : "/"}${path}`;
}
