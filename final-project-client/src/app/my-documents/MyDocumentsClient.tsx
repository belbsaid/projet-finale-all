...
export default function MyDocumentsClient() {
  ...
  const allDocuments = rawDocs.map((doc) => ({
    ...doc,
    carLabel: doc.car ? `${doc.car.brand?.name || ''} ${doc.car.model?.name || ''} ${doc.car.year ? doc.car.year : ''}`.trim() : 'Véhicule',
  })).filter((doc: any) => {
    if (!q) return true;
  ...
  return (
    <div className="container animate-fade-in-up" style={{ padding: '2rem 1.5rem 4rem' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <div>
          <h1 className="section-title" style={{ marginBottom: '0.25rem' }}>{t('myDocuments')}</h1>
      ...
      {isLoading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {allDocuments.map((doc) => (
            <div key={doc._id} className="glass-card" style={{ padding: '1.25rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '2rem' }}>{DOC_ICONS[doc.type] || '📄'}</span>
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontWeight: 600, fontSize: '0.9375rem' }}>{DOC_LABELS[doc.type] || doc.name}</h3>
        ...
        <div className="glass-card" style={{ padding: '3rem', textAlign: 'center' }}>
          <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>📂</div>
          <h2 style={{ fontWeight: 700, marginBottom: '0.5rem' }}>Aucun document disponible</h2>
          <p style={{ color: 'var(--text-secondary)', maxWidth: '400px', margin: '0 auto 1.5rem' }}>
            Vos documents (COC, facture, garantie) apparaîtront ici une fois votre véhicule livré.
          </p>
          <Link href="/my-cars" className="btn-secondary" style={{ display: 'inline-flex' }}>
            {t('myCars')}
          </Link>
        </div>
      )}
