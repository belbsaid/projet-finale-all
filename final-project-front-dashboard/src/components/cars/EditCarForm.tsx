...
export function EditCarForm({ carId, defaultValues }: EditCarFormProps) {
  ...
  const onSubmit = async (data: CarFormData) => {
    ...
    try {
      ...
      await carsApi.update(
        carId,
        payload as unknown as Record<string, unknown>,
      );
      
      // If there are new pending photos, upload them
      if (pendingPhotos.length > 0) {
        const fd = new FormData();
        pendingPhotos.forEach((f) => fd.append("photos", f));
        try {
          await carsApi.uploadPhotos(carId, fd);
        } catch {
          toast.error("Car updated, but failed to upload new photos");
        }
      }
